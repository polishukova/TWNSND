export const Plus = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M2.75 12C2.75 11.4477 3.19772 11 3.75 11H20.25C20.8023 11 21.25 11.4477 21.25 12C21.25 12.5523 20.8023 13 20.25 13H3.75C3.19772 13 2.75 12.5523 2.75 12Z"
        fill="white"
      />
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M12 2.75C12.5523 2.75 13 3.19772 13 3.75V20.25C13 20.8023 12.5523 21.25 12 21.25C11.4477 21.25 11 20.8023 11 20.25V3.75C11 3.19772 11.4477 2.75 12 2.75Z"
        fill="white"
      />
    </svg>
  )
}
